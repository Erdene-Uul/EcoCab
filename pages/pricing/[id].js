...
const PricingDetail = () => {
  ...
  useEffect(() => {
    if (selectedPlace) {
      console.log(
        `Хаягийн мэдээлэл: ${selectedPlace.lat}, ${selectedPlace.lng}`
      );
      setFormData({
        ...formData,
  ...
  const handleChange = (e) => {
    ...
    // Conditionally set carType based on the name of the input field
    const carType = ['firstname', 'lastname', 'phoneNumber', 'email'].includes(
      name
    )
      ? pricing.mark
      : formData.carType;

    // Update formData with price based on selected tab
    const price =
      router.query.tab === '0'
      ...
      <div className="max-w-7xl w-full lg:p-4 p-0">
        <div className="md:flex lg:p-4 p-2">
          ...
          <div className="md:w-1/2 md:pl-6 mt-6 md:mt-0">
            ...
            <p className="text-gray-700 mb-4 leading-relaxed">{pricing.desc}</p>
            <p className="text-gray-600 mb-2 leading-relaxed">{pricing.text1}</p>
            <p className="text-gray-600 mb-2 leading-relaxed">{pricing.text2}</p>
          </div>
        </div>
        <div className="border rounded-3xl md:p-6 p-2 mt-6 bg-white">
          <div
            // onSubmit={}
            className="grid grid-cols-1 md:grid-cols-2 gap-3 py-4"
          >
            ...
            <div className="md:col-span-1 mt-6">
              ...
              <div className="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-3 rounded-sm ">
                ...
                <p className="text-[12px]">
                  Хаяг болон байршлын мэдээлэлд алдаа гарсан тохиолдолд, доорх
                  мэдээллийг анхаарна уу
                </p>
              </div>
            </div>
            <div className="md:col-span-2">
