...
const PricingInfo = () => {
  ...
  return (
    <div className='w-full py-4 cursor-default'>
    ...
    <div className='text-gray-800 h-full grid lg:grid-cols-3 grid-cols-2 gap-5'> 
      {pricings.types.map((pricing, index) => (
        <div key={index} className='relative group transition-all overflow-hidden duration-300 ease-in-out bg-[#fff] border border-[#f2f2f2] rounded-2xl p-5 h-full hover:border-gray-300'>
          <div className='w-full flex items-center flex-col '>  
                  <p className='uppercase font-semibold text-gray-500 text-xs text-center'>{pricing.mark}</p>
                  <img className="" src={pricing.image} width={250} alt="" />
           ...
           <div className='absolute left-0 top-1/2 transition-all duration-500 ease-in-out rounded-lg group-hover:-translate-y-1/2 opacity-0 group-hover:opacity-100  bg-white '>
            <p className='font-light px-2 py-2 pt-10 text-gray-700 text-xs md:text-justify text-center' >{pricing.text1}</p>
            <p className='font-light px-2 py-2 pb-10 text-gray-700 text-xs md:text-justify text-center'>{pricing.text2}</p>
          </div> 
        </div>
      ))}
    </div>
